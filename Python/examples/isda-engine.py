    ...
    for spread in spreads:
        for recovery in recoveries:
            ...
            h = quotedTrade.impliedHazardRate(
                0, discountCurve, ql.Actual365Fixed(), recovery, 1e-10, ql.CreditDefaultSwap.ISDA
            )

            probabilityCurve.linkTo(
                ql.FlatHazardRate(0, ql.WeekendsOnly(), ql.makeQuoteHandle(h), ql.Actual365Fixed())
            )
...
if not interactive:
    print(df)
df.style.format(
    {"Spread": "{:.4%}", "Hazard rate": "{:.2%}", "Upfront": "{:.2f}", "Markit value": "{:.2f}", "Distance": "{:.6f}"}
)
