...
EU_FIXING_DATA = [
    (ql.Date(1, ql.April, 2018), 103.11),
...
def build_nominal_term_structure(reference_date, nominal_data):
...
def build_hicp_index(fixing_data, inflation_crv_handle):
...
def build_inflation_term_structure(
    ...
    helpers = [
        create_inflation_swap_helper(reference_date, x, inflation_index, interpolation) for x in zero_coupon_swaps_data
    ]
    cpi_term_structure = ql.PiecewiseZeroInflation(
        reference_date, inflation_index.lastFixingDate(), inflation_index.frequency(), DAY_COUNTER, helpers
    )
...
def interpolate_historic_index(inflation_idx, fixing_date, observation_lag=OBSERVATION_LAG):
    ...
    slope_denominator = (second_dt + observation_lag) - (first_dt + observation_lag)
    slope = float(slope_numerator) / float(slope_denominator)
    return inflation_idx.fixing(first_dt) + slope * (inflation_idx.fixing(second_dt) - inflation_idx.fixing(first_dt))
...
class InflationTest(unittest.TestCase):
    def setUp(self):
        ...
        self.nominal_ts_handle.linkTo(build_nominal_term_structure(VALUATION_DATE, EUR_ZERO_RATES))
    ...
    def test_par_swap_pricing_fom_indexation_without_seasonality(self):
        ...
        inflation_idx = build_hicp_index(EU_FIXING_DATA, self.inflation_ts_handle)
        inflation_ts = build_inflation_term_structure(VALUATION_DATE, EUR_BEI_SWAP_RATES, inflation_idx, ql.CPI.Flat)
        self.inflation_ts_handle.linkTo(inflation_ts)

        zciis = create_inflation_swap(
            inflation_idx, VALUATION_DATE, CAL.advance(VALUATION_DATE, ql.Period(10, ql.Years)), 0.0355, ql.CPI.Flat
        )
        ...
        fail_msg = """ Failed to price zero coupon inflation swap to par:
                            ...
                            tolerance: {tolerance}
                   """.format(
            inflation_idx=inflation_idx.familyName(),
        ...
        self.assertTrue(abs(npv) < EPSILON, msg=fail_msg)
    ...
    def test_inflation_leg_payment_fom_indexation_without_seasonality(self):
        ...
        inflation_cf = ql.as_indexed_cashflow(zciis.inflationLeg()[0])
        ...
        effective_fixing_dt = ql.Date(1, fixing_dt.month(), fixing_dt.year())
        fraction = inflation_ts.dayCounter().yearFraction(ts_base_dt, effective_fixing_dt)
        t = inflation_ts.timeFromReference(effective_fixing_dt)
        zero_rate = inflation_ts.zeroRate(t)
        expected_fixing = ts_base_fixing * (1.0 + zero_rate) ** fraction

        expected_inflation_leg_payment = (expected_fixing / swap_base_fixing - 1.0) * inflation_cf.notional()
        ...
        self.assertAlmostEqual(
            first=actual_inflation_leg_payment, second=expected_inflation_leg_payment, delta=EPSILON, msg=fail_msg
        )
    ...
    def test_lagged_fixing_method(self):
        ...
        self.assertAlmostEqual(first=actual_fixing, second=expected_fixing, msg=fail_msg, delta=EPSILON)
