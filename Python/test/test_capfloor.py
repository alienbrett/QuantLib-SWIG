...
class CapFloorTest(unittest.TestCase):
    def setUp(self):
        ...
        self.flat_forward = ql.FlatForward(
            self.today_date, self.flat_forward_rate, self.rate_day_counter, ql.Continuous, ql.Annual
        )
        ...
        self.ibor_leg = ql.IborLeg([self.notional], self.schedule, self.ibor_index)
        self.strike = 0.01
    ...
    def testBlackCapFloorEngine(self):
        ...
        black_engine = ql.BlackCapFloorEngine(self.term_structure_handle, self.black_vol)

        self.cap.setPricingEngine(black_engine)
        npv = self.cap.NPV()
        self.assertAlmostEqual(npv, self.cap_npv, places=1, msg="NPV method is broken")
        vol_guess = 0.5
        imp_vol = self.cap.impliedVolatility(npv, self.term_structure_handle, vol_guess)
        self.assertAlmostEqual(self.black_vol.value(), imp_vol, places=4, msg="Implied volatility method is broken")

    def testBachelierCapFloorEngine(self):
        """Testing BachelierCapFloorEngine"""

        bpvol = self.black_vol.value() * self.flat_forward_rate
        bachelier_engine = ql.BachelierCapFloorEngine(self.term_structure_handle, ql.makeQuoteHandle(bpvol))
        ...
        imp_vol = self.cap.impliedVolatility(self.cap_npv, self.term_structure_handle, vol_guess, type=ql.Normal)

        self.assertAlmostEqual(bpvol, imp_vol, places=4, msg="Normal Implied volatility method is broken")
