...
SMILE_OPT_TENORS = (ql.Period(1, ql.Years), ql.Period(10, ql.Years), ql.Period(30, ql.Years))

SMILE_SWAP_TENORS = (ql.Period(2, ql.Years), ql.Period(10, ql.Years), ql.Period(30, ql.Years))
...
def build_euribor_swap_idx(projection_curve_handle):
    return ql.EuriborSwapIsdaFixA(ql.Period(1, ql.Years), projection_curve_handle)


def build_nominal_term_structure(valuation_date, nominal_quotes):
    dates, rates = zip(*[(CAL.advance(valuation_date, x[0]), x[1]) for x in nominal_quotes])
...
def build_linear_swaption_cube(
    ...
    vol_spreads = [[ql.makeQuoteHandle(v) for v in row] for row in vol_spreads]
...
def sabr_parameters_guess(number_of_options, number_of_swaps):
    ...
    for n in range(n_elements):
        guess[n] = (ql.makeQuoteHandle(0.2), ql.makeQuoteHandle(0.5), ql.makeQuoteHandle(0.4), ql.makeQuoteHandle(0.0))
    return guess
...
def build_sabr_swaption_cube(
    ...
    v_spreads = [[ql.makeQuoteHandle(v) for v in row] for row in vol_spreads]
    guess = sabr_parameters_guess(len(spread_opt_tenors), len(spread_swap_tenors))
...
class SwaptionVolatilityCubeTest(unittest.TestCase):
    ...
    def _assert_atm_strike(self, cube, interpolation, vol_type):
        ...
        self.assertAlmostEqual(first=actual_atm_strike, second=expected_atm_strike, delta=TOLERANCE, msg=fail_msg)

    def _assert_atm_vol(self, cube, opt_tenor, swap_tenor, expected_vol, interpolation, vol_type, epsilon=TOLERANCE):
        ...
        self.assertAlmostEqual(first=actual_vol, second=expected_vol, delta=epsilon, msg=fail_msg)

    def _assert_vol_spread(
        self, cube, opt_tenor, swap_tenor, strike_spread, expected_vol, interpolation, vol_type, epsilon=TOLERANCE
    ):
        option_date = cube.optionDateFromTenor(opt_tenor)
    ...
    def test_linear_normal_cube_at_the_money_strike(self):
        ...
        linear_cube = build_linear_swaption_cube(
            NORM_VOL_MATRIX, SMILE_OPT_TENORS, SMILE_SWAP_TENORS, STRIKE_SPREADS, NORM_VOL_SPREADS, self.swap_idx
        )
        self._assert_atm_strike(cube=linear_cube, interpolation="linear", vol_type="normal")
    ...
    def test_linear_lognormal_cube_at_the_money_strike(self):
        ...
        self._assert_atm_strike(cube=linear_cube, interpolation="linear", vol_type="log-normal")
    ...
    def test_sabr_lognormal_cube_at_the_money_strike(self):
        ...
        self._assert_atm_strike(cube=sabr_cube, interpolation="SABR", vol_type="log-normal")
...
class AndreasenHugeVolatilityTest(unittest.TestCase):
    def testLocalVolCalibration(self):
        ...
        local_vol = ql.LocalVolTermStructureHandle(
            ql.AndreasenHugeLocalVolAdapter(ql.AndreasenHugeVolatilityInterpl(calibration_set, spot, rTS, qTS))
        )
        ...
        option.setPricingEngine(
            ql.MCEuropeanEngine(
                local_vol_process, "lowdiscrepancy", timeSteps=75, brownianBridge=True, requiredSamples=16000, seed=42
            )
        )
        ...
        expected = ql.BlackCalculator(
            ql.as_plain_vanilla_payoff(option.payoff()), fwd, vol * math.sqrt(t), rTS.discount(t)
        ).value()
