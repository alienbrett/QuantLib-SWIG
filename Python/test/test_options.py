...
class OptionsTest(unittest.TestCase):
    def testFdHestonHullWhite(self):
        ...
        option = ql.VanillaOption(
            ql.PlainVanillaPayoff(ql.Option.Call, s0), ql.EuropeanExercise(todays_date + ql.Period(365, ql.Days))
        )
        ...
        option.setPricingEngine(
            ql.FdHestonHullWhiteVanillaEngine(
                ql.HestonModel(heston_process), hull_white_process, -0.5, 10, 200, 25, 10, 0, True
            )
        )
    ...
    def testAnalyticHestonHullWhite(self):
        ...
        option = ql.VanillaOption(ql.PlainVanillaPayoff(ql.Option.Call, 100.0), ql.EuropeanExercise(maturityDate))
        ...
        heston_model = ql.HestonModel(ql.HestonProcess(r, q, underlying, v0, kappa, theta, sigma, rho))

        option.setPricingEngine(ql.AnalyticHestonHullWhiteEngine(heston_model, hull_white_model))
        self.assertAlmostEqual(expected, option.NPV(), 5)

        option.setPricingEngine(ql.AnalyticH1HWEngine(heston_model, hull_white_model, 0.0))
        self.assertAlmostEqual(expected, option.NPV(), 5)
    ...
    def testCashDividendEuropeanEngine(self):
        ...
        option.setPricingEngine(
            ql.CashDividendEuropeanEngine(process, div_schedule, ql.CashDividendEuropeanEngine.Escrowed)
        )
        ...
        option.setPricingEngine(
            ql.CashDividendEuropeanEngine(process, div_schedule, ql.CashDividendEuropeanEngine.Spot)
        )
