...
def flat_rate(rate):
    return ql.FlatForward(LAG, CAL, ql.makeQuoteHandle(rate), DC)
...
class PerpetualFuturesTest(unittest.TestCase):
    def test_perpetual_futures(self):
        ...
        test_dicts = [
            # Discrete time
            {
                "payoff": ql.PerpetualFutures.Linear,
                "funding": ql.PerpetualFutures.FundingWithPreviousSpot,
                "freq": ql.Period(3, ql.Months),
                "spot": 10000.0,
                "domRate": 0.04,
                "btcYield": 0.02,
                "fundingRate": 0.01,
            ...
            {
                "payoff": ql.PerpetualFutures.Linear,
                "funding": ql.PerpetualFutures.FundingWithCurrentSpot,
                "freq": ql.Period(3, ql.Months),
                "spot": 10000.0,
                "domRate": 0.04,
                "btcYield": 0.02,
            ...
            {
                "payoff": ql.PerpetualFutures.Inverse,
                "funding": ql.PerpetualFutures.FundingWithPreviousSpot,
                "freq": ql.Period(3, ql.Months),
                "spot": 10000.0,
                "domRate": 0.04,
        ...
        for d in test_dicts:
            ...
            ir_diffs = [d["rateDiff"]]
            engine = ql.DiscountingPerpetualFuturesEngine(
                domYC,
                btcYC,
                spot,
                fundingTimes,
            ...
            if unit == ql.Years:
                ...
                dt = length / 365.0 / 24.0
            elif unit == ql.Minutes:
                dt = length / 365.0 / 24.0 / 60.0
            elif unit == ql.Seconds:
                dt = length / 365.0 / 24.0 / 60.0 / 60.0
            else:
            ...
            if length > 0:
                if d["payoff"] == ql.PerpetualFutures.Linear:
                    if d["funding"] == ql.PerpetualFutures.FundingWithPreviousSpot:
                        ...
                        expected = (
                            d["spot"]
                            * (d["fundingRate"] - d["rateDiff"])
                            * math.exp(d["btcYield"] * dt)
                            / (
                                math.exp(d["btcYield"] * dt)
                                - math.exp(d["domRate"] * dt)
                                + d["fundingRate"] * math.exp(d["btcYield"] * dt)
                            )
                        )
                        ...
                        expected = (
                            ...
                            * math.exp(d["domRate"] * dt)
                            / (
                                math.exp(d["btcYield"] * dt)
                                - math.exp(d["domRate"] * dt)
                                + d["fundingRate"] * math.exp(d["domRate"] * dt)
                            )
                        )
                ...
                if d["payoff"] == ql.PerpetualFutures.Linear:
                    ...
                    expected = (
                        d["spot"]
                        * (d["fundingRate"] - d["rateDiff"])
                        / (d["btcYield"] - d["domRate"] + d["fundingRate"])
                    )
                elif d["payoff"] == ql.PerpetualFutures.Inverse:
                    # "Proposition 4" in the above paper
                    expected = (
                        d["spot"]
                        * (d["domRate"] - d["btcYield"] + d["fundingRate"])
                        / (d["fundingRate"] - d["rateDiff"])
                    )
