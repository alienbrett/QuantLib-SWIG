...
OPTION_TYPE_MAP = {ql.Swap.Receiver: "Receiver", ql.Swap.Payer: "Payer"}

SETTLEMENT_TYPE_MAP = {ql.Settlement.Physical: "Physical", ql.Settlement.Cash: "Cash"}

SETTLEMENT_METHOD_MAP = {
    ql.Settlement.PhysicalOTC: "Physical OTC",
    ql.Settlement.CollateralizedCashPrice: ("Collateralized Cash Price"),
    ql.Settlement.ParYieldCurve: "Par Yield Curve",
}


def compounded_annual_constant_rate_discount(rate, day_counter):
    def _calc(start, end):
...
def par_yield_bps(underlying, discount_handle):
    ...
    bps = sum(
        [
            ir_func(discount_date, c_f.date()) * c_f.amount() / fixed_rate
...
class SwaptionTest(unittest.TestCase):
    def setUp(self):
        ...
        projection_curve = ql.FlatForward(self.today, self.projection_quote_handle, ql.Actual365Fixed())
        projection_curve_handle.linkTo(projection_curve)

        self.discount_handle = ql.YieldTermStructureHandle(
            ql.FlatForward(self.today, ql.makeQuoteHandle(0.0085), ql.Actual365Fixed())
        )
    ...
    def _assert_swaption_annuity(self, swaption_pricer_func, use_bachelier_vol):
        self.projection_quote_handle.linkTo(ql.SimpleQuote(self.projection_rate))

        settle_type = ql.Settlement.Cash
        methods = [ql.Settlement.ParYieldCurve, ql.Settlement.CollateralizedCashPrice]
            ...
            for l in self.lengths:
                for t in self.swap_type:
                    for m in methods:
                        ...
                        strike = 0.03
                        swaption_engine = swaption_pricer_func(self.discount_handle, volatility)
                        exercise_date = self.calendar.advance(self.today, e)
                        start_date = self.calendar.advance(exercise_date, ql.Period(2, ql.Days))
                        ...
                        swaption = ql.Swaption(underlying, ql.EuropeanExercise(exercise_date), settle_type, m)
                        ...
                        if m == ql.Settlement.ParYieldCurve:
                            expected_annuity = par_yield_bps(underlying, self.discount_handle)
                        ...
                        self.assertAlmostEqual(first=annuity, second=expected_annuity, delta=EPSILON, msg=fail_msg)

    def test_swaption_annuity_black_model(self):
        """Testing swaption annuity in Black model"""
        self._assert_swaption_annuity(swaption_pricer_func=make_const_black_vol_engine, use_bachelier_vol=False)

    def test_swaption_annuity_bachelier_model(self):
        """Testing swaption annuity in Bachelier model"""
        self._assert_swaption_annuity(swaption_pricer_func=make_const_bachelier_vol_engine, use_bachelier_vol=True)
